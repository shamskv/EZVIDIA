...
GlobalConfiguration NvapiDriver::_getConfig() {
  ...
  do {
    ...
    NvAPI_Status ret = NvAPI_DISP_GetDisplayConfig(&pInfoWrapper.nDisplays,
                                                   pInfoWrapper.pInfo);
    if (ret != NVAPI_OK) {
      LOG(ERR) << "Problem calling NvAPI_DISP_GetDisplayConfig. Code: " << ret;
      throw DriverException(
          L"Error calling NvAPI_DISP_GetDisplayConfig (code " +
  ...
  LOG(DEBUG) << "NvAPI_DISP_GetDisplayConfig result contains nDisplays == "
             << pInfoWrapper.nDisplays;
  for (uint32_t i = 0; i < pInfoWrapper.nDisplays; i++) {
    LOG(DEBUG) << "NvAPI_DISP_GetDisplayConfig Display " << i << " contains "
               << pInfoWrapper.pInfo[i].targetInfoCount << " targetInfoCounts";
    for (uint32_t j = 0; j < pInfoWrapper.pInfo[i].targetInfoCount; j++) {
      ...
      dConf.colorDepth =
          pInfoWrapper.pInfo[i].sourceModeInfo->resolution.colorDepth;
      ...
      dConf.refresh =
          pInfoWrapper.pInfo[i].targetInfo[j].details->refreshRate1K;
...
bool NvapiDriver::_applyConfig(const GlobalConfiguration& conf) {
  ...
  for (auto& disp : conf.displays) {
    ...
    // This means if two displays in the same clone group have different source
    // modes it will silently pick one (undefined)
    ...
    pInfoWrapper.pInfo[i].targetInfo[j].details->rotation =
        static_cast<NV_ROTATE>(disp.rotation);
    pInfoWrapper.pInfo[i].targetInfo[j].details->scaling =
        static_cast<NV_SCALING>(disp.scaling);
    pInfoWrapper.pInfo[i].targetInfo[j].details->tvFormat =
        static_cast<NV_DISPLAY_TV_FORMAT>(disp.tvFormat);
  }
  pInfoWrapper.pInfo[groupIndexMap[conf.primaryGroup]]
      .sourceModeInfo->bGDIPrimary = 1;

  NvAPI_Status ret =
      NvAPI_DISP_SetDisplayConfig(pInfoWrapper.nDisplays, pInfoWrapper.pInfo,
                                  NV_DISPLAYCONFIG_SAVE_TO_PERSISTENCE);
  if (ret != NVAPI_OK) {
    /*throw DriverException(L"Error calling NvAPI_DISP_SetDisplayConfig (code "
     * + std::to_wstring(ret) + L")");*/
