...
bool NvapiPathInfoWrapper::allocate() {
  ...
  // First pass (allocate according nDisplays * pInfo and respective source mode
  // infos
  if (this->pInfo == nullptr) {
    this->pInfo = (NV_DISPLAYCONFIG_PATH_INFO*)calloc(
        this->nDisplays, sizeof(NV_DISPLAYCONFIG_PATH_INFO));
    if (this->pInfo == nullptr) {
    ...
    for (uint32_t i = 0; i < this->nDisplays; i++) {
      ...
      this->pInfo[i].sourceModeInfo =
          (NV_DISPLAYCONFIG_SOURCE_MODE_INFO*)calloc(
              1, sizeof(NV_DISPLAYCONFIG_SOURCE_MODE_INFO));
      if (this->pInfo[i].sourceModeInfo == nullptr) {
        throw DriverException(
            L"Error allocating memory (sourceModeInfo calloc).");
      }
    }
    return true;
  }
  ...
  for (uint32_t i = 0; i < this->nDisplays; i++) {
    if (this->pInfo[i].targetInfo == nullptr &&
        this->pInfo[i].targetInfoCount > 0) {
      this->pInfo[i].targetInfo = (NV_DISPLAYCONFIG_PATH_TARGET_INFO*)calloc(
          this->pInfo[i].targetInfoCount,
      ...
      for (uint32_t j = 0; j < this->pInfo[i].targetInfoCount; j++) {
        this->pInfo[i].targetInfo[j].details =
            (NV_DISPLAYCONFIG_PATH_ADVANCED_TARGET_INFO*)calloc(
                1, sizeof(NV_DISPLAYCONFIG_PATH_ADVANCED_TARGET_INFO));
        if (this->pInfo[i].targetInfo[j].details == nullptr) {
          throw DriverException(
              L"Error allocating memory (targetInfo.details calloc).");
        }
        this->pInfo[i].targetInfo[j].details->version =
