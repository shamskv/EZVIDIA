...
template <typename T>
inline bool isValueInJson(const nlohmann::json& root,
...
template <>
inline bool isValueInJson<int32_t>(const nlohmann::json& root,
                                   const std::string& name) {
  return root.contains(name) && root[name].is_number();
}

template <>
inline bool isValueInJson<std::string>(const nlohmann::json& root,
                                       const std::string& name) {
  return root.contains(name) && root[name].is_string();
}

template <>
inline bool isValueInJson<bool>(const nlohmann::json& root,
                                const std::string& name) {
...
T getValueFromJson(const nlohmann::json& root, const std::string& name,
                   std::optional<T> defaultValue = std::optional<T>()) {
...
bool JsonSettings::read() {
  ...
  int schema_version =
      getValueFromJson(fRoot, "schema_version", std::optional<int>(1));
  ...
  if (schema_version == 1) {
    ...
    if (fRoot.contains("configList") && fRoot["configList"].is_array()) {
      for (auto& gConfig : fRoot["configList"]) {
        ...
        if (std::find_if(configVector.begin(), configVector.end(),
                         [&conf](GlobalConfiguration& gc) {
                           return gc.name == conf.name;
                         }) != configVector.end()) {
...
GlobalConfiguration jsonToGlobalConfig(const nlohmann::json& gcRoot) {
  ...
  // Name
  globalConf.name = StringUtils::stringToWideString(
      getValueFromJson<std::string>(gcRoot, "name"));
  ...
  for (auto& dispRoot : gcRoot["displayList"]) {
    try {
      ...
      std::wstring errorMsg(
          L"Error occured while parsing the displayList for configuration \"");
  ...
  // Clone groups
  // If one of the displays in the configuration had a missing cloneGroup value
  // (which is represented by a UINT32_MAX), we assume extended mode
  if (std::find_if(globalConf.displays.begin(), globalConf.displays.end(),
                   [](const DisplayConfiguration& disp) {
                     return disp.cloneGroup == UINT32_MAX;
                   }) != globalConf.displays.end()) {
    uint32_t group = 0;
  ...
  // Primary Group (in configurations with primaryId, we change it to the group
  // containing that id)
  globalConf.primaryGroup = getValueFromJson<uint32_t>(
      gcRoot, "primaryGroup", std::optional<uint32_t>(UINT32_MAX));
  if (globalConf.primaryGroup == UINT32_MAX) {
    uint32_t primaryId = getValueFromJson<uint32_t>(
        gcRoot, "primaryId", std::optional<uint32_t>(UINT32_MAX));
    for (auto& disp : globalConf.displays) {
  ...
  if (!validPrimaryGroup) {
    std::wstring errorMsg(
        L"Error occured while parsing primaryGroup/Id for configuration \"");
    errorMsg.append(globalConf.name);
    errorMsg.append(L"\":\n");
    errorMsg.append(
        L"Provided primaryGroup/Id doesn't match any of the "
        L"cloneGroups/displayIds in DisplayList");
    throw ConfException(errorMsg);
  }
...
DisplayConfiguration jsonToDisplayConfig(const nlohmann::json& dcRoot) {
  ...
  // Other parameters are optional and have a default value (for compatibility
  // with legacy config files)
  dispConf.scaling =
      getValueFromJson<uint32_t>(dcRoot, "scaling", std::optional<uint32_t>(0));
  dispConf.tvFormat = getValueFromJson<uint32_t>(dcRoot, "tvFormat",
                                                 std::optional<uint32_t>(0));
  dispConf.cloneGroup = getValueFromJson<uint32_t>(
      dcRoot, "cloneGroup", std::optional<uint32_t>(UINT32_MAX));

  return dispConf;
}

nlohmann::json jsonFromDisplayConfig(
    const DisplayConfiguration& displayConfig) {
