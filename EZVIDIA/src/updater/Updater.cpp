...
std::optional<version_array_t> extractVersion(std::wstring versionString) {
  ...
  for (int i = 0; i < std::tuple_size<version_array_t>::value; i++) {
    ...
    if (versionStream.fail()) {
      if (i == 0)
        return std::nullopt;  // If we couldnt even read a number it's a fail
      else
        break;
    }

    if (versionStream.get() != '.')
      break;
  }
...
template <class T>
std::optional<T> downloadAsset(const nlohmann::json& assetArray,
                               const std::string assetName) {
  auto assetIt =
      std::find_if(assetArray.begin(), assetArray.end(), findAsset(assetName));
  ...
  try {
    ...
    LOG(ERR) << "Problem getting download url/content type for "
             << assetName.c_str() << " with exception: " << e.what();
    return std::nullopt;
  }
...
std::optional<VersionInfo> Updater::getLatestVersion() {
  ...
  std::string versionUrl("https://api.github.com/repos/" + Updater::owner +
                         "/" + Updater::repo + "/releases/latest");
  std::string versionContentType("application/vnd.github.v3+json");

  auto optionalVersionJson = HttpClient::makeGetRequestSsl<nlohmann::json>(
      versionUrl, versionContentType);
  ...
  try {
    ...
    info.tag = StringUtils::stringToWideString(
        versionJson["tag_name"].get<std::string>());
    info.notes =
        StringUtils::stringToWideString(versionJson["body"].get<std::string>());
    info.assetsUrl = versionJson["assets_url"].get<std::string>();
    return info;
  } catch (std::exception& e) {
    LOG(ERR)
        << "Problem extracting tag + notes from latest release. exception msg: "
        << e.what();
  }
...
bool Updater::downloadAndInstall(const VersionInfo& version) {
  ...
  auto optionalAssetArray = HttpClient::makeGetRequestSsl<nlohmann::json>(
      version.assetsUrl, assetArrayContentType);

  if (!optionalAssetArray) {
    LOG(ERR) << "Problem downloading asset array with url "
             << version.assetsUrl.c_str();
    return false;
  }
  ...
  auto optionalZip =
      downloadAsset<std::vector<char>>(assetArray, "ezvidia.zip");
