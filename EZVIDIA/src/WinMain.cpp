...
int APIENTRY wWinMain(_In_ HINSTANCE hInstance,
  ...
  std::optional<std::wstring>
      confNameToApply;  // if set, we try to apply this config and exit app
  ...
  if (argv != NULL) {
    if (argc == 1 && wcslen(lpCmdLine) > 0) {
      ...
      MessageBox(NULL,
                 L"Error starting application: invalid arguments\nRun without "
                 L"arguments or "
                 "with 1 argument containing the desired configuration name "
  ...
  std::unique_ptr<Settings> config =
      std::make_unique<JsonSettings>("configs.json");
  ...
  if (confNameToApply.has_value()) {
    ...
    if (optionalConf.has_value()) {
      ...
      MessageBox(NULL,
                 L"Error applying configuration.\nConfiguration not found.",
                 NULL, MB_OK | MB_ICONERROR | MB_APPLMODAL);
      return -1;
    }
  }
